...
/**
 * Create the engine (see docs/api.md). Register handlers with `on()` or consume with `subscribe()`, then
...
class WatukuyEngine<P extends PollerMap> implements Engine<P> {
  ...
  private async refreshPartitions(rt: PollerRuntime, force = false): Promise<Partition<unknown>[]> {
    ...
    // Pause partitions that disappeared (see docs/multi-tenant.md: paused, not deleted).
