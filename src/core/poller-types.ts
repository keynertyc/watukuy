...
/** One tenant / account / shard of a poller (see docs/multi-tenant.md). */
...
/** Argument of a poller `fetch` function (see docs/api.md). */
export interface FetchContext<Cursor, PData = undefined> {
  ...
  lane: Lane;
  /** HTTP helper with ETag, rate-limit parsing, budget charging (see docs/http-helper.md). Optional to use. */
  http: HttpClient;
  signal: AbortSignal;
...
/** Polling cadence and adaptation (see docs/how-it-works.md). */
...
/** Circuit breaker thresholds (see docs/how-it-works.md). */
...
/** Delivery retry policy (see docs/delivery.md). */
...
/** What happens when delivery attempts are exhausted (see docs/delivery.md). */
...
/** Periodic full-listing sweep that detects deletes for incremental strategies (see docs/cursors.md). */
...
/** Options common to `definePoller` with and without a schema (see docs/api.md). */
