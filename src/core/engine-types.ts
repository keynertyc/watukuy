...
/** A named, shared rate budget declared in `createWatukuy({ budgets })` (see docs/how-it-works.md). */
...
/** Options for `createWatukuy()` (see docs/api.md). */
...
/** Options for `engine.backfill()` (see docs/runbook.md). */
...
/** The running engine (see docs/api.md). Create with `createWatukuy()`. */
export interface Engine<P extends PollerMap> {
  ...
  /** One pass over due pollers, then return (serverless mode, see docs/serverless.md). */
